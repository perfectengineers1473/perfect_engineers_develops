...
const NavbarSection: React.FC<NavbarSectionProps> = ({
  ...
  return (
    <nav
      className={`flex ${
        mobile ? "flex-col gap-4" : "items-center gap-2"
      }`}
    >
      {buttonlinks?.map((btn, index) => (
        <Link
          ...
          className={`
            ...
            bg-gray-100/70 hover:bg-transparent
          `}
        >
          ...
          {/* Glow */}
          <span className="absolute inset-0 rounded-xl shadow-[0_0_0_0_rgba(99,102,241,0.4)] group-hover:shadow-[0_10px_30px_-10px_rgba(168,85,247,0.6)] transition-all" />
